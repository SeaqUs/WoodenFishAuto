...
- **空闲检测**：用 Win32 `GetLastInputInfo` 精确判断「鼠标键盘连续 30 秒无输入」。
  关键特性：`SendInput` 注入的模拟按键**不会**刷新空闲计时，因此程序自己刷的功德不会误判成"用户已回来"。
- **自动刷功德**：用户空闲时自动模拟键盘连续输入字符（游戏全局监听键盘，任何窗口输入都算功德），
  输入到本程序面板窗口，不污染用户其它应用；用户一动鼠标/键盘立即停止。
...
- `type_delay_ms`：模拟按键间隔，默认 25 毫秒（约 40 键/秒）
