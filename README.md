...
- **自动刷功德**：两种方式可单选切换——**点击木鱼**（已验证有效）或**键盘输入 note.ms**。
  用户一动鼠标/键盘立即停止。

> **实测结论**：该游戏会过滤"合成键盘输入"（`SendInput` 注入的按键不计功德），
> 但合成鼠标点击有效。因此「点击木鱼」方式可用；「键盘输入」方式按你的要求实现，
> 但注入的按键可能同样被游戏过滤、不涨功德，需你实测确认。
> 木鱼中心位置用配置 `fish_center_rel` 固定（已验证）。
